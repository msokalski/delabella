...
struct IDelaBella3
{
	...
	struct Vertex
	{
		Vertex* next; // next in internal / boundary set of vertices
		Simplex* sew; // one of triangles sharing this vertex
		T x, y, z;    // coordinates (input copy / sphere projection approximation)
		I i; // index of original point
	...
	virtual const Simplex* GetFirstSimplex() const = 0; // valid only if Triangulate() > 0
	virtual const Vertex*  GetFirstBoundaryVertex() const = 0; // vertices used in triangulation
	virtual const Vertex*  GetFirstInternalVertex() const = 0; // vertices consumed by convex hull
	...
	// returns number of 'land' faces (they start at GetFirstDelaunaySimplex)
	// optionally <exterior> pointer is set to the first 'sea' face
	// if invert is set, seed face and its neighbors will become 'land' (instead of 'sea')
	virtual I FloodFill(const Simplex* seed, bool invert, const Simplex** exterior = 0) = 0;
	...
	// next N <x>,<y> elements will contain edge normals (divisor W=0)
	// function returns number vertices filled (V) on success, otherwise 0
	virtual I GenVoronoiDiagramVerts(T* x, T* y, T* z, size_t advance_bytes = 0) const = 0;
