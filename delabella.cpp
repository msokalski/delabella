...
static int Test()
{
	...
	rlo = _mul128(a, b, &rhi); // _umul128( unsigned )

	unsigned char carry = 0;
	unsigned __int64 ru;
	carry = _addcarry_u64(carry, a, b, &ru);
	...
	*/
	...
	//for (int sr = 9; sr < 100000000; sr++) 
	{
		...
		for (int i = 0; i < 2 * POINTS; i++)
		{
			...
			unsigned int frac = (rand() << 15) | rand(); // 30 bits
			xy[i] = quot + frac / (double)(1<<30);
		}

		LARGE_INTEGER t0, t1, fr;
		QueryPerformanceCounter(&t0);
		verts = DelaBella(POINTS, xy, abc);
		QueryPerformanceCounter(&t1);

		QueryPerformanceFrequency(&fr);

		double dt = t1.QuadPart - t0.QuadPart;
		dt /= (double)fr.QuadPart;
	...
	for (int i = 0; i < POINTS; i++)
	{
		double x = xy[2 * i + 0];
		double y = xy[2 * i + 1];
		fprintf(f, "{%f,%f}%s", x, y, i == POINTS - 1 ? "}\n" : ",\n");
	}
	...
	for (int i = 0; i < tris; i++)
	{
		...
		int b = abc[3 * i + 1];
		int c = abc[3 * i + 2];
		fprintf(f, "{{%f,%f},{%f,%f}},",  xy[2 * a + 0], xy[2 * a + 1], xy[2 * b + 0], xy[2 * b + 1]);
		fprintf(f, "{{%f,%f},{%f,%f}},",  xy[2 * b + 0], xy[2 * b + 1], xy[2 * c + 0], xy[2 * c + 1]);
		fprintf(f, "{{%f,%f},{%f,%f}}%s", xy[2 * c + 0], xy[2 * c + 1], xy[2 * a + 0], xy[2 * a + 1], i==tris-1 ? "}\n":",\n");
	}

	fprintf(f, "Graphics[{Line[eds], Point[pts]},Axes->True]\n");
	fclose(f);
